...
class StorageUtils {
    ...
    companion object {
        ...
        fun hasInternetPermission(context: Context): Boolean {
            var permission =
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    PermissionChecker.checkSelfPermission(context, Manifest.permission.INTERNET)
                }
            return permission == PackageManager.PERMISSION_GRANTED
        }
         ...
         * REQUIRES NETWORK ACCESS AND WRITE ACCESS TO THE STORAGE
        ...
        fun downloadDummyImages(context: Context, number: Int) {
            ...
            /**
             * Checking permission
             * if permission access the internet has not been granted throw SecurityException
             */
            if (!hasInternetPermission(context = context))
                throw SecurityException(context.getString(R.string.permission_internet_not_granted))
